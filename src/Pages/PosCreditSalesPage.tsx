...
import { CustomerInt, MultiSelectOptionInt, PaymentType, ProductInt, SalesInt, SalesType } from '../Interfaces';
...
import { Payment } from '.';
import { useClientPaymentPrinter, useCreditPurchaseClientPrinter } from '../hooks';
...
export const CreditSalesPage = () => {
  ...
  const [selectedProductDataOption, setSelectedProductDataOption] = useState<any[]>([]);
  ...
  const [stats, setStats] = useState<StatsInt>({ totalAmount: 0.0, salesCount: 0 });
  ...
  const { handlePrint: handlePaymentPrint, PrintComponent: PaymentPrintComponent } = useClientPaymentPrinter()
  ...
  const changeQuantity = (index: number, qty?: string) => {
    ...
    if (qty === undefined || qty === '' || qty === 'e' || isNaN(parseInt(qty))) {
  ...
  const changePrice = (index: number, price?: string) => {
    const productsCopy = [...selectedProducts];
    if (price === undefined || price === '' || price === 'e' || isNaN(parseFloat(price))) {
  ...
  const calculateSubTotal = (_prods: ProductInt[]) => {
    let _subTotal = 0.0;
    _prods.forEach((prod) => {
      const rowTotal = parseInt(`${prod?.quantity || 0}`) * parseFloat(`${prod?.price || 0}`) || 0.0;
  ...
  const changeAmountPaid = (amt: string) => {
    if (amt === undefined || amt === '' || amt === 'e' || isNaN(parseFloat(amt))) {
    ...
    if (salesHist?.status === true) {
      ...
      if (isPrintable === true) {
        if (salesHist?.data?.length > 0) {
          handlePrint(salesHist?.data?.at(-1));
    ...
    <>
      <Flex className='non-printable'>
        <VStack minH={'100vh'} minW="60vw" >
              ...
              <Tbody>

                {
                  selectedProducts?.map((prod, index, arr) => {
                    ...
                    return <Tr key={'row-credit-sales-key'.concat(`${index}`)}>
                      <Td isNumeric>
                        {index + 1}.
                      </Td>
                      <Td >
                      ...
                      <Td >
                        <Input id={`quantity:${prod?.id}`} textAlign={"end"} type={"number"} name="quantity" placeholder='0.0' onChange={(e) => changeQuantity(index, e.target.value || '0')} defaultValue={parseInt(`${prod?.quantity || 0}`)} />
                      </Td>
                      <Td >
                        <Input id={`price:${prod?.id}`} textAlign={"end"} type={"number"} name="price" placeholder='0.0' onChange={(e) => changePrice(index, e.target.value || '0')} defaultValue={parseFloat(`${prod?.price || 0}`)} />
                      </Td>
              ...
              {selectedProducts?.length !== 0 && <Tfoot>
                ...
                <Tr>
                  ...
                  <Th borderColor={"transparent"} isNumeric>
                    <Input borderColor={"initial"} placeholder='0.0' textAlign={"end"} type={"number"} name="amountPaid" onChange={(e) => changeAmountPaid(e.target.value)} />
                  </Th>
                </Tr>
                {amountPaid > 0 && <Tr >
                  <Th borderColor={"transparent"} colSpan={3} isNumeric>Balance</Th>
                  <Th borderColor={"transparent"} color={balance < 0 ? 'red' : 'ButtonText'} colSpan={0} isNumeric>GH&#162; {parseFloat(`${balance || 0.0}`).toFixed(2)}</Th>

                </Tr>}
                <Tr >
                  <Th borderColor={"transparent"} colSpan={3} isNumeric>Subtotal</Th>
                  <Th borderColor={"transparent"} color={'ButtonText'} colSpan={0} isNumeric>GH&#162; {parseFloat(`${subTotal}`)?.toFixed(2) || 0.0}</Th>

                </Tr>
                <Tr borderColor={"transparent"}>

                  <Th colSpan={0} isNumeric><Button onClick={onToggle} isActive={false} disabled={selectedProducts?.length === 0}>Cancel</Button></Th>
                  <Th colSpan={3} isNumeric><Button disabled={selectedProducts?.length === 0} isActive={true} onClick={handleSubmit}>Submit & Print Receipt</Button></Th>
                </Tr>
              </Tfoot>}
            </Table>
          ...
          <Payment onOpen={paymentAlertProps.onOpen} onClose={paymentAlertProps.onClose} isOpen={paymentAlertProps.isOpen} data={customersDataOptions} selected={selectedCustomerDataOption} customer={customers?.filter((val) => val?.id == selectedCustomerDataOption?.at(0)?.key)?.at(0)} customers={customers}
            callback={(val: Partial<PaymentType>) => {
              fetchCustomers();
              handlePaymentPrint(val)
            }} />
          ...
          <CustomAlertDialog onOpen={onOpen} isOpen={isOpen} onClose={onClose} onToggle={onToggle} message="Are you sure ?" title='Remove Data' callback={resetData} isDelete />

          <CustomAlertDialog onOpen={submitDisclosure?.onOpen} isOpen={submitDisclosure?.isOpen} onClose={submitDisclosure?.onClose} onToggle={submitDisclosure.onToggle} message="Are you sure ?" title='Submit Data' callback={submitData} buttonText={'Submit'} />
        </VStack>
        <VStack>
          <Flex>
            <VStack>
              <Text fontWeight={"bold"}>{parseFloat(`${stats?.totalAmount || 0.0}`).toFixed(2)}</Text>
              <Text>Amount(GH&#162;)</Text>
            </VStack>
            <Spacer minW={'10'} />
            <VStack>
            ...
            <Tbody>
              {
                salesHistory?.map((sale, index, arr) =>
                  <Tr key={`credit-sales-history-${index}`} >
                    ...
                    <Td>
                      <IconButton onClick={() => handlePrint(sale as Partial<SalesType>)} icon={<FaPrint />} aria-label={'print'} />
                    </Td>
                  </Tr>
                )
              }
